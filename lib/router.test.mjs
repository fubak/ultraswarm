...
describe('router', () => {
  ...
  describe('resolveRoute', () => {
    ...
    it('small-harness moderate/complex/expert tiers route to claude models via BACKEND env var', () => {
      ...
      assert.match(moderate.command, /^BACKEND=openrouter AGENT_MODEL=anthropic\/claude-sonnet-4-6/);

      const complex = resolveRoute({ cli: 'small-harness', model_tier: 'complex' });
      assert.match(complex.command, /^BACKEND=openrouter AGENT_MODEL=anthropic\/claude-opus-4-8/);

      const expert = resolveRoute({ cli: 'small-harness', model_tier: 'expert' });
      assert.match(expert.command, /^BACKEND=openrouter AGENT_MODEL=anthropic\/claude-opus-4-8/);
    });
