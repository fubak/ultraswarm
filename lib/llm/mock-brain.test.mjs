...
describe('mockBrainBehavior', () => {
  ...
  // Test hook: a QA_REJECT sentinel in the prompt forces a critical refutation, so e2e tests can
  // deterministically drive the adversarial-QA-rejection (and O3 retry) path with the real runner.
  it('refutes critically when the prompt carries the QA_REJECT sentinel', () => {
    const { object } = mockBrainBehavior('verify:t1:security', { schema: EXPERT_VERDICT_SCHEMA, prompt: 'review this: QA_REJECT please' })
    assert.equal(object.refuted, true)
    assert.equal(object.severity, 'critical')
    assert.ok(object.reasons.length > 0)
  })
