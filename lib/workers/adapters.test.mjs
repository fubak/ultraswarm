...
test('parseUsage reads REAL structured usage from codex/opencode JSONL (not a text scrape)', () => {
  // WHY: usage must come from the worker's structured JSONL event — never a free-text scrape (the old
  ...
  assert.deepEqual(adapter.parseUsage(codex), { input_tokens: 16810, output_tokens: 5, totalTokens: 16815, costUsd: null })
  ...
  assert.deepEqual(adapter.parseUsage(oc), { input_tokens: 4095, output_tokens: 107, totalTokens: 4202, costUsd: 0.012 })
  // Multiple events SUM (total tokens the worker processed across turns/steps).
  assert.equal(adapter.parseUsage(`${codex}\n${codex}`).totalTokens, 33630)
})
