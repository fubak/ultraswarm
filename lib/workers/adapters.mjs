...
export class ShellWorkerAdapter {
  ...
  // Real token/cost usage from a worker's STRUCTURED JSONL output — never a free-text scrape (the old
  // /tokens?\s*used?[:\s]+(\d+)/ matched incidental numbers in a worker's own content and undercounted
  // by orders of magnitude). The codex (`exec --json`) and opencode (`run --format json`) invocations
  // emit one usage event per turn/step; we parse and SUM them (total tokens the worker actually
  // processed — what the provider bills). Two known shapes:
  //   codex:    {"type":"turn.completed","usage":{"input_tokens":N,"output_tokens":M,...}}
  //   opencode: {"type":"step_finish",...,"part":{"tokens":{"input":N,"output":M,...},"cost":C}}
  // A worker that emits neither (text-mode CLI, fake test worker) yields nulls → the report says
  // "not reported" instead of inventing a figure (Rule 12).
  parseUsage(text) {
    ...
    for (const raw of (text || '').split('\n')) {
      ...
      const cu = o.usage   // codex: top-level usage object
      if (cu && (typeof cu.input_tokens === 'number' || typeof cu.output_tokens === 'number')) {
        input += cu.input_tokens || 0; output += cu.output_tokens || 0; found = true
      }
      const ot = o.part?.tokens ?? o.tokens   // opencode: nested under .part; tolerate top-level too
      if (ot && (typeof ot.input === 'number' || typeof ot.output === 'number')) {
        input += ot.input || 0; output += ot.output || 0; found = true
        const c = o.part?.cost ?? o.cost
