...
// HTTP auth header: mask the entire value after "authorization:" (covers "Authorization: Bearer <key>"
// and any other scheme). Requires the ':' delimiter so ordinary prose ("authorization department")
// is not touched.
const AUTH_HEADER_RE = /\bauthorization\s*:\s*.+/ig
// Bare "Bearer <token>" scheme. Require a token-shaped value (>=8 token chars) so prose like
// "the bearer of bad news" is left alone while opaque bearer tokens are still masked.
const BEARER_SCHEME_RE = /\bbearer\s+[A-Za-z0-9._~+/=-]{8,}/ig
...
export const redact = (text = '') => {
  ...
  out = out.replace(BEARER_SCHEME_RE, 'bearer [REDACTED]')
