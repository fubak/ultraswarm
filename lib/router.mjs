 ...
 * merge with the base) and carries maxTier through. The binary falls back through
 * alias.binary -> base.binary -> the base CLI name (alias.extends) -> the alias name, so an
 * alias extending a built-in that declares no explicit binary still resolves to a usable one.
