...
function validateModels(cli, models, errors, scope = 'overrides') {
  if (!models || typeof models !== 'object' || Array.isArray(models)) {
    errors.push(`${scope}.${cli}.models must be an object; got ${JSON.stringify(models)}.`);
    return;
  }
  if (!Object.hasOwn(models, 'simple')) {
    errors.push(`${scope}.${cli}.models is missing required tier "simple" as the fallback anchor.`);
  }
  for (const [tier, entry] of Object.entries(models)) {
    if (!VALID_TIER_SET.has(tier)) {
      errors.push(`${scope}.${cli}.models has invalid tier "${tier}".`);
      continue;
    }
    if (typeof entry?.model !== 'string' || entry.model.trim() === '') {
      errors.push(`${scope}.${cli}.models.${tier}.model must be a non-empty string; got ${JSON.stringify(entry?.model)}.`);
    }
    validateInvocation(entry?.invocation, `${scope}.${cli}.models.${tier}.invocation`, errors);
  }
}

function validateUsage(usage, location, errors) {
  if (!Array.isArray(usage)) {
    errors.push(`${location} must be an array; got ${JSON.stringify(usage)}.`);
    return;
  }
  usage.forEach((descriptor, index) => {
    if (!descriptor || typeof descriptor !== 'object' || Array.isArray(descriptor)) {
      errors.push(`${location}[${index}] must be an object; got ${JSON.stringify(descriptor)}.`);
      return;
    }
    if (typeof descriptor.input !== 'string' || descriptor.input.trim() === '') {
      errors.push(`${location}[${index}].input must be a non-empty string; got ${JSON.stringify(descriptor.input)}.`);
    }
    if (typeof descriptor.output !== 'string' || descriptor.output.trim() === '') {
      errors.push(`${location}[${index}].output must be a non-empty string; got ${JSON.stringify(descriptor.output)}.`);
    }
    if (Object.hasOwn(descriptor, 'cost') && typeof descriptor.cost !== 'string') {
      errors.push(`${location}[${index}].cost must be a string when present; got ${JSON.stringify(descriptor.cost)}.`);
    }
  });
}

function validateOverride(cli, override, errors, warnings, aliasNames = new Set()) {
  ...
  if (Object.hasOwn(override, 'usage')) {
    validateUsage(override.usage, `overrides.${cli}.usage`, errors);
  }
}
...
export const DEFAULT_REGISTRY = freezeDeep({
  codex: {
    ...
    strengths: ['backend', 'logic', 'debugging', 'architecture'],
    structuredOutput: true,
    resume: true,
    models: {
      simple: {
        model: 'gpt-5.4-mini',
        invocation: 'codex exec --json -s workspace-write --skip-git-repo-check -m gpt-5.4-mini {{EFFORT}}"$(cat .ultraswarm-prompt.txt)" </dev/null'
      },
      moderate: {
        model: 'gpt-5.4',
        invocation: 'codex exec --json -s workspace-write --skip-git-repo-check -m gpt-5.4 {{EFFORT}}"$(cat .ultraswarm-prompt.txt)" </dev/null'
      },
      complex: {
        model: 'gpt-5.5',
        invocation: 'codex exec --json -s workspace-write --skip-git-repo-check -m gpt-5.5 {{EFFORT}}"$(cat .ultraswarm-prompt.txt)" </dev/null'
      },
      expert: {
        model: 'gpt-5.5',
        invocation: 'codex exec --json -s workspace-write --skip-git-repo-check -m gpt-5.5 {{EFFORT}}"$(cat .ultraswarm-prompt.txt)" </dev/null'
  ...
  gemini: {
    ...
    timeoutMs: 600000,
    usage: [{ input: 'stats.models.*.tokens.prompt', output: 'stats.models.*.tokens.candidates' }],
    ...
    models: {
      simple: {
        ...
        invocation: 'gemini --yolo --output-format json -m gemini-2.5-flash -p "$(cat .ultraswarm-prompt.txt)"'
      },
      moderate: {
        model: 'gemini-2.5-pro',
        invocation: 'gemini --yolo --output-format json -m gemini-2.5-pro -p "$(cat .ultraswarm-prompt.txt)"'
      },
      complex: {
        model: 'gemini-2.5-pro',
        invocation: 'gemini --yolo --output-format json -m gemini-2.5-pro -p "$(cat .ultraswarm-prompt.txt)"'
  ...
  grok: {
    ...
    timeoutMs: 600000,
    strengths: ['tests', 'refactors', 'general'],
    structuredOutput: false,
    resume: false,
    models: {
      simple: {
  ...
  agy: {
    ...
    timeoutMs: 600000,
    strengths: ['docs', 'boilerplate', 'automation'],
    structuredOutput: false,
    resume: false,
    models: {
      simple: {
  ...
  droid: {
    ...
    strengths: ['full-stack', 'refactors', 'architecture'],
    structuredOutput: false,
    resume: false,
    models: {
      simple: {
  ...
  opencode: {
    ...
    strengths: ['boilerplate', 'lint', 'tests', 'docs'],
    structuredOutput: false,
    resume: false,
    models: {
      simple: {
        model: 'xai/grok-build-0.1',
        invocation: 'opencode run --format json --agent build -m "xai/grok-build-0.1" "$(cat .ultraswarm-prompt.txt)"'
      },
      moderate: {
        model: 'xai/grok-4.3',
        invocation: 'opencode run --format json --agent build -m "xai/grok-4.3" "$(cat .ultraswarm-prompt.txt)"'
      },
      complex: {
        model: 'google/gemini-3.1-pro-preview',
        invocation: 'opencode run --format json --agent build -m "google/gemini-3.1-pro-preview" "$(cat .ultraswarm-prompt.txt)"'
      },
      expert: {
        model: 'xai/grok-4.20-0309-reasoning',
        invocation: 'opencode run --format json --agent build -m "xai/grok-4.20-0309-reasoning" "$(cat .ultraswarm-prompt.txt)"'
      }
    }
  },
  pi: {
    ...
    strengths: ['general', 'full-stack', 'refactors'],
    structuredOutput: false,
    resume: false,
    models: {
      simple: {
  ...
  'pi-local': {
    ...
    timeoutMs: 900000,
    binary: 'pi',
    strengths: ['general', 'boilerplate', 'docs', 'tests'],
    structuredOutput: false,
    resume: false,
    models: {
      simple: {
  ...
  'small-harness': {
    specialty: 'tool-rich agent with MCP integration, multi-backend support (cloud and local models), and cost tracking',
    timeoutMs: 900000,
    binary: 'small-harness',
    strengths: ['tool-rich', 'mcp-integration', 'cost-tracking', 'multi-backend', 'local-models'],
    structuredOutput: false,
    resume: false,
    models: {
      simple: {
        model: 'gpt-4o-mini',
        invocation: 'BACKEND=openai AGENT_MODEL=gpt-4o-mini small-harness --allow-tools --print "$(cat .ultraswarm-prompt.txt)"'
      },
      moderate: {
        model: 'claude-sonnet-4-6',
        invocation: 'BACKEND=openrouter AGENT_MODEL=anthropic/claude-sonnet-4-6 small-harness --allow-tools --print "$(cat .ultraswarm-prompt.txt)"'
      },
      complex: {
        model: 'claude-opus-4-8',
        invocation: 'BACKEND=openrouter AGENT_MODEL=anthropic/claude-opus-4-8 small-harness --allow-tools --print "$(cat .ultraswarm-prompt.txt)"'
      },
      expert: {
        model: 'claude-opus-4-8',
        invocation: 'BACKEND=openrouter AGENT_MODEL=anthropic/claude-opus-4-8 small-harness --allow-tools --print "$(cat .ultraswarm-prompt.txt)"'
  ...
  agent: {
    specialty: 'general full-stack, refactors, IDE-native agentic workflows',
    timeoutMs: 900000,
    binary: 'agent',
    strengths: ['general', 'full-stack', 'refactors', 'debugging', 'tests'],
    structuredOutput: false,
    resume: true,
    models: {
      simple: {
        model: 'composer-2.5-fast',
        invocation: 'agent -p --force --output-format text --model composer-2.5-fast "$(cat .ultraswarm-prompt.txt)"'
      },
      moderate: {
        model: 'gpt-5.4',
        invocation: 'agent -p --force --output-format text --model gpt-5.4 "$(cat .ultraswarm-prompt.txt)"'
      },
      complex: {
        model: 'claude-sonnet-4-6',
        invocation: 'agent -p --force --output-format text --model claude-sonnet-4-6 "$(cat .ultraswarm-prompt.txt)"'
      },
      expert: {
        model: 'claude-opus-4-8',
        invocation: 'agent -p --force --output-format text --model claude-opus-4-8 "$(cat .ultraswarm-prompt.txt)"'
      }
    }
  }
});
 ...
 * effortFlags, usage, strengths, structuredOutput, resume, modelListCmd, and specialty
 * (when omitted); the alias OWNS its models map (no per-tier merge with the base) and
 * carries maxTier through. The binary falls back through alias.binary -> base.binary ->
 * the base CLI name (alias.extends) -> the alias name, so an alias extending a built-in
 * that declares no explicit binary still resolves to a usable one.
 * Assumes the alias has already passed validateAliases — buildRegistry never resolves an
 * alias with a bad/missing base.
 */
function resolveAlias(name, alias) {
  ...
  return {
    ...
    usage: alias.usage ?? base.usage,
    strengths: alias.strengths ?? base.strengths,
    structuredOutput: alias.structuredOutput ?? base.structuredOutput,
    resume: alias.resume ?? base.resume,
    modelListCmd: alias.modelListCmd ?? base.modelListCmd,
...
function validateAliases(aliases, errors) {
  ...
  for (const [name, alias] of Object.entries(aliases)) {
    ...
    validateModels(name, alias.models, errors, 'aliases');
    if (Object.hasOwn(alias, 'usage')) {
      validateUsage(alias.usage, `aliases.${name}.usage`, errors);
    }
  }
}
...
export function validateConfig(config) {
  ...
  for (const [cli, override] of Object.entries(candidate.overrides ?? {})) {
    validateOverride(cli, override, errors, warnings, aliasNames);
  }
