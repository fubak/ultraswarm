...
export function buildReport({ merged = [], failed = [], blocked = [], externalTokens = 0, attempts = {}, taskCount, tokenCoverage, workerAttempts = 0, externalWallMs = 0, cliUsage = [], routeUsage = [], runWallMs = 0, markdown = false } = {}) {
  ...
  // Ledger invariant: every planned task must appear in exactly one terminal bucket. A mismatch
  // means a task was silently dropped somewhere — say so loudly instead of printing plausible
  // numbers that don't reconcile (Rule 12).
  const accounted = merged.length + failed.length + blocked.length
  if (taskCount != null && accounted !== taskCount) {
    const warn = `⚠ LEDGER MISMATCH: ${accounted} task rows but the plan had ${taskCount} — a task was lost; inspect with \`ultraswarm status <run>\`.`
    lines.push(markdown ? `**${warn}**` : c.red(warn))
  }
  ...
  // The report ENDS with the estimated-vs-used breakdown by CLI × model × effort. `est.` is a
  // calibration-informed heuristic (per planned task); `used` is ONLY structured usage summed over
  // every attempt in that bucket — "—" when the CLI reported nothing, never backfilled from the
  // estimate. Δ appears only when both sides exist.
  if (routeUsage.length) {
    lines.push('', h2('Tokens by CLI / model / effort'))
    const delta = (est, used) => (est > 0 && used != null ? `${used >= est ? '+' : ''}${Math.round(((used - est) / est) * 100)}%` : '—')
    const rows = routeUsage.map((u) => [u.cli, u.model, u.effort, u.estimated > 0 ? num(u.estimated) : '—', u.used != null ? num(u.used) : '—', delta(u.estimated, u.used), u.attempts])
    const estTotal = routeUsage.reduce((s, u) => s + u.estimated, 0)
    const usedRows = routeUsage.filter((u) => u.used != null)
    if (routeUsage.length > 1) {
      const usedTotal = usedRows.length ? usedRows.reduce((s, u) => s + u.used, 0) : null
      const estOfUsed = usedRows.reduce((s, u) => s + u.estimated, 0)
      rows.push(['Total', '', '', num(estTotal), usedTotal != null ? num(usedTotal) : '—', delta(estOfUsed, usedTotal), routeUsage.reduce((s, u) => s + u.attempts, 0)])
    }
    lines.push(table(['CLI', 'model', 'effort', 'est.', 'used', 'Δ', 'attempts'], rows, ['left', 'left', 'left', 'right', 'right', 'right', 'right']))
    const unreported = routeUsage.length - usedRows.length
    const note = `est. = calibration/heuristic estimate per planned task; used = structured usage across all attempts${unreported ? `; "—" = ${unreported} route(s) reported no usage` : ''}.`
    lines.push(markdown ? `_${note}_` : c.dim(note))
  }
  return lines.join('\n')
}
