...
export function buildReport({ merged = [], failed = [], blocked = [], externalTokens = 0, attempts = {}, taskCount, tokenCoverage, workerAttempts = 0, externalWallMs = 0, cliUsage = [] } = {}) {
  ...
  // Offload headline — a QUALITATIVE value statement, never a fabricated token count. Worker token
  // self-reports are unreliable (most external CLIs emit none), so the concrete, measured numbers
  // (attempts, wall-clock) live in the "Work offloaded" section below; the headline just states the
  // value proposition that's always true: the implementation ran off Claude's context.
  const offloadLine = total === 0 ? null
    : `**Implementation ran on external CLIs — off your Claude context.** Claude ran orchestration + QA review only.`
  ...
  if (cliUsage.length && spentTotal > 0) {
    ...
    lines.push('', '| CLI | attempts | landed | spent | overhead |', '|---|--:|--:|--:|--:|')
    for (const u of cliUsage) {
      const oh = u.spent - u.landed
      lines.push(`| ${u.cli} | ${u.attempts} | ${num(u.landed)} | ${num(u.spent)} | ${oh > 0 ? '+' : ''}${num(oh)} |`)
    }
    if (cliUsage.length > 1) {
      const aTot = cliUsage.reduce((s, u) => s + u.attempts, 0)
      lines.push(`| **Total** | ${aTot} | ${num(landedTotal)} | ${num(spentTotal)} | +${num(overheadTotal)} |`)
    }
