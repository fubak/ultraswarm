...
function validateCliName(cli, extraAllowed = []) {
  if (!cli || typeof cli !== 'string') {
    throw new Error('CLI name must be a non-empty string')
  }
  // Accept built-in CLIs plus any caller-supplied names (user-defined aliases / legacy registry
  // entries), so alias workers can be retried in the attempt loop — not just compete (#alias-compete).
  if (!ALLOWED_CLIS.includes(cli) && !extraAllowed.includes(cli)) {
    throw new Error(`Invalid CLI name: ${cli}. Allowed: ${[...ALLOWED_CLIS, ...extraAllowed].join(', ')}`)
  }
  return cli
}
...
// ── effort-first escalation ladder ────────────────────────────────────────────
// Climb effort within a tier (low→medium→high), then step the tier and reset
// effort to low. 'off'/'xhigh' are off-ladder (indexOf -1) → fall through to a
// tier step. Maxed at expert@high.
...
// Effort-only escalation rung — climbs low→medium→high, capped at high. Used by the routine
// task loop (which keeps its model tier and only varies compute). `off` climbs into the ladder;
// `xhigh` is already above the ladder so it stays put.
export function nextEffort(effort) {
  const e = EFFORT_LADDER.indexOf(effort)
  if (e === -1) return effort === 'off' ? 'low' : effort
  return e < EFFORT_LADDER.length - 1 ? EFFORT_LADDER[e + 1] : effort
}
...
export function createOrchestrator({ agent, parallel, log, cfg, runImplementation = _runImplementation }) {
  ...
  // A cli is usable if it's a known worker (resolvable via DEFAULT_REGISTRY + overrides), a
  // user-defined alias, or has an explicit legacy registry entry. Aliases route fine for routine
  // tasks via resolveRoute/buildRegistry, so they MUST also be eligible for high-risk competition —
  // omitting cfg.aliases here silently tombstoned alias-pinned high-risk tasks as "only N usable
  // worker(s)". The high-risk path must NOT gate on cfg.registry alone — the documented
  // `enabled`/`overrides`/`aliases` config has no legacy `registry` map (issues #6 / #13).
  const cliUsable = (c) => !!c && (ALLOWED_CLIS.includes(c) || !!(cfg.aliases && cfg.aliases[c]) || !!(cfg.registry && cfg.registry[c]))
   ...
   * @param {string|null} startEffort - Effort to begin at (carries escalation across CLI reassignment)
   * @returns {Promise<object>} success {task, cli, impl, attempts, feedback, final_model_tier, final_effort, complexity_achieved}
   *                            or exhaustion {exhausted:true, feedback, final_model_tier, final_effort}
   */
  async function intelligentAttemptLoop(t, cli, maxAttempts, seedFeedback, attemptOffset = 0, startTier = null, startEffort = null) {
    try {
      const validCli = validateCliName(cli, [...Object.keys(cfg.aliases || {}), ...Object.keys(cfg.registry || {})])
      ...
      for (let n = 1; n <= maxAttempts; n++) {
        ...
        if (verdict.approve) return {
          ...
          final_model_tier: currentModelTier, final_effort: currentEffort, complexity_achieved: impl.complexity_achieved,
        }
      ...
      return { exhausted: true, feedback, final_model_tier: currentModelTier, final_effort: currentEffort }
    } catch (error) {
      log(`Attempt loop failed for ${t.id}: ${error.message}`)
      return { exhausted: true, feedback: [...seedFeedback, `Attempt loop error: ${error.message}`], final_model_tier: t.model_tier || 'simple', final_effort: t.effort || DEFAULT_EFFORT }
    }
  }

  // ── judge competition between implementations ──────────────────────────────
  async function judgeCompetition(t, impls) {
    // #S6: winner defaults to impls[0] — used when there's a single impl, or when all judges die
    // (logged in the catch below). This is intentional: the winner is still gated by adaptiveQA
    // downstream, so a judge outage degrades to "first impl, still QA-verified", not a silent bypass.
  ...
  async function handleFailedCompetition(t, all, winner = null, qaIssues = []) {
    const retryCli = winner ? winner.cli : t.cli
    const seed = [
      // #O3: a winner that passed gates but was rejected by QA leaves every impl status:'ok', so the
      // gate-failure seed below is empty — forward the QA rejection reasons so the retry isn't blind.
      ...qaIssues.map(issue => `prior winner rejected by QA: ${issue}`),
    ...
    const fallback = await intelligentAttemptLoop(t, altCli, 2,
      [...retried.feedback, `prior CLI (${retryCli}) exhausted`], 3, retried.final_model_tier, retried.final_effort)
  ...
  async function runCompetitiveTask(t) {
    try {
      ...
      // Quorum: a high-risk task REQUIRES competition (≥2 usable workers + a judge). With only one
      // usable worker there is no alternate to compete against and no judge — approving a solo impl
      // would silently bypass the adversarial path. Tombstone cleanly instead of approving solo.
      if (t.risk === 'high' && competitors.length < 2) {
        const reason = `competition required but only ${competitors.length} usable worker(s)`
        log(`${t.id}: ${reason} — tombstoning`)
        return { task: t.id, failed: true, error: reason }
      }
      ...
      if (impls.length === 0) {
        log(`✗ ${t.id}: all ${all.length} competitor(s) failed gates — retrying`)
        return await handleFailedCompetition(t, all)
      }

      const { winner, graft } = await judgeCompetition(t, impls)
      // Show how the competition resolved — which impl the judge picked — so the winner that QA then
      // reviews isn't a mystery (only meaningful when >1 impl actually competed).
      if (impls.length > 1) log(`${t.id}: judged winner ${winner.cli} of ${impls.length} competitors that passed gates`)
      ...
      if (winner) {
        ...
        if (verdict.approve) {
          return {
            ...
            // #O4: report the task's resolved tier — runImplementation hardcodes model_used:'external'.
            final_model_tier: t.model_tier, complexity_achieved: winner.complexity_achieved,
          }
        }
        qaIssues = verdict.issues || []   // #O3: forward the QA rejection so the retry isn't blind
        // Make the QA rejection VISIBLE: the winner passed its gates (implement.mjs logged "✓ gates"),
        // so without this line the live stream jumps straight from "✓ gates" to a fresh "attempt 2"
        // with no reason — the silent-retry gap. Mirror the routine path's "✗ … rejected" line.
        log(`✗ ${t.id}: competition winner (${winner.cli}) rejected by QA (${qaIssues.length} issue${qaIssues.length === 1 ? '' : 's'}) — retrying`)
      }

      return await handleFailedCompetition(t, all, winner, qaIssues)
    } catch (error) {
  ...
  async function runStandardTask(t) {
    try {
      ...
      // Carry the escalated tier into the alternate CLI (don't restart at the task's base tier)
      const fallback = await intelligentAttemptLoop(t, altCli, 2,
        [...primary.feedback, `prior CLI (${t.cli}) exhausted`], 3, primary.final_model_tier, primary.final_effort)
      return fallback.exhausted ? { task: t.id, failed: true } : fallback
    } catch (error) {
