...
import { buildWorkerTaskPrompt, capWorkerPrompt } from '../prompts.mjs'
...
function resolveTaskRoute(cfg, t, cli) {
  ...
  // when no legacy entry is present for the cli. The legacy path has no resolved model
  // id, so it keeps the historical tier-as-model attribution.
  const entry = cfg.registry && cfg.registry[cli]
  if (entry) {
    const command = typeof entry === 'string' ? entry : (entry[t.model_tier] || entry.simple)
    return { command, model: t.model_tier, tier: t.model_tier ?? null, effort: t.effort ?? null }
  }
  // Modern resolution (preferred for enabled/overrides configs from bin/runner) — carries the
  // RESOLVED model id + tier + effort so attempts are attributable per (cli, model, effort).
  return resolveRoute({ ...t, cli }, cfg)
}
...
export const allowedEnv = (cfg = {}) => {
  // Enumerate the XDG vars workers actually need rather than passing the whole XDG_* namespace — a
  // wildcard leaks things like a credential-dir override (XDG_*_HOME pointing at secrets) to every
  // third-party worker CLI (#SE4). Operators can add more via cfg.workerEnvAllowlist.
  const names = new Set(['HOME', 'PATH', 'SHELL', 'TERM', 'TMPDIR', 'LANG', 'LC_ALL', 'USER', 'LOGNAME',
    'XDG_CONFIG_HOME', 'XDG_CACHE_HOME', 'XDG_DATA_HOME', 'XDG_RUNTIME_DIR', ...(cfg.workerEnvAllowlist ?? [])])
  return Object.fromEntries(Object.entries(process.env).filter(([key]) => names.has(key)))
}
...
export async function runImplementation(cfg, t, cli, attempt, feedback) {
  ...
  try {
    // Budget gate. NOTE: read-then-act — with N parallel workers the cap can be overrun by up to one
    // in-flight attempt per worker before costs land at finishAttempt (#ST3). Costs are recorded
    // accurately post-hoc; tighten to an atomic reservation at startAttempt if a hard cap is required.
    ...
    // resolveTaskRoute can throw for an unknown cli/tier — keep it INSIDE the try so the task returns a
    // loud cli_failed result instead of propagating (which pipeline() would swallow → silent task loss).
    const route = resolveTaskRoute(cfg, t, cli)
    ...
    if (!fs.existsSync(wt)) {
      ...
      // Capture git's stderr (don't inherit it): `worktree add` prints "Preparing worktree …" per
      // task, which otherwise floods the live progress stream and buries the ▶/✔ lines on a big swarm.
      execFileSync('git', ['worktree', 'add', wt, '-b', br, cfg.baseBranch], { cwd: cfg.repo, stdio: ['ignore', 'pipe', 'pipe'] })
    ...
    const prompt = capWorkerPrompt(buildWorkerTaskPrompt(t, feedback))
    if (prompt.truncated) log(`⚠ ${t.id}: worker prompt truncated (${prompt.truncated} chars over the cap)`)
    fs.writeFileSync(path.join(wt, '.ultraswarm-prompt.txt'), prompt.text)
    ...
    try {
        ...
        if (cfg.workerManager) {
          supervised = await cfg.workerManager.get(cli).execute({ task: t, cwd: wt, timeoutMs, env: allowedEnv(cfg), label: `${cfg.runId ?? 'run'}-${t.id}-${attempt}`,
            onStart: ({ pid, logPath }) => {
              ...
              if (cfg.store && cfg.runId) attemptId = cfg.store.startAttempt({ runId: cfg.runId, taskId: t.id, number: attempt, worker: cli, model: route.model, tier: route.tier, effort: route.effort, pid, logPath })
            } })
      ...
      // A failed attempt still burned real tokens — record its structured usage (when the adapter
      // captured any) so the report's "spent" column counts failures, not just landed work.
      const failUsage = e.supervised?.usage ?? {}
      const failTokens = (failUsage.input_tokens ?? 0) + (failUsage.output_tokens ?? 0)
      if (attemptId) cfg.store.finishAttempt(attemptId, { status: 'failed', exitCode: e.supervised?.code, durationMs: e.supervised?.durationMs, errorKind: kind,
        inputTokens: failUsage.input_tokens ?? null, outputTokens: failUsage.output_tokens ?? null, costUsd: failUsage.costUsd ?? (priceUsd(route.model, failUsage, cfg) || null) })
      if (failTokens > 0) cfg.store?.recordCalibration?.({ cli, model: route.model, effort: route.effort ?? 'low', tier: route.tier ?? 'simple', tokens: failTokens })
      ...
      resetWorktree(wt)
      return impl('cli_failed', wt, br, [], [], `worker ${cli} failed (${kind})${workerErrorHint(kind, cli)}: ${e.message}`, 0)
    }
    ...
    if (preGateChanged.length === 0) {
      ...
      return impl('no_changes', wt, br, [], [], `CLI produced no file changes on attempt ${attempt} on ${cli}`, 0)
    }
    ...
    if (forbidden.length) {
      ...
      return impl('policy_blocked', wt, br, [], [], `forbidden path written: ${forbidden.join('; ')}`, 0)
    }
    ...
    try { execFileSync('git', ['commit', '-q', '-m', `ultraswarm: ${t.id}/${t.model_tier} attempt ${attempt}`], { cwd: wt }) }
      ...
      return impl('cli_failed', wt, br, changed, gate_results, `success commit failed for ${t.id} (#S3): ${String(e.stderr || e.message).slice(0, 300)}`, 0)
    }
    ...
    // Token/cost usage comes ONLY from a worker's structured usage object — never a free-text scrape
    // of its stdout (that matched incidental numbers and undercounted by orders of magnitude). Most
    // external CLIs expose nothing here, so these stay null/0 and the report says "not reported"
    // rather than inventing a figure (#ST5). cli_tokens is the structured total (0 when none) so the
    // run's externalTokens reflects only trustworthy usage.
    const usage = supervised?.usage ?? {}
    const inputTokens = usage.input_tokens ?? usage.inputTokens ?? null
    const outputTokens = usage.output_tokens ?? usage.outputTokens ?? usage.totalTokens ?? null
    const reportedTokens = (inputTokens ?? 0) + (outputTokens ?? 0)
    if (attemptId) {
      // Worker USD cost: adapter-reported when available; otherwise derived from the structured
      // token counts × operator-configured rates (config.intelligence.pricing — priceUsd returns 0
      // for unknown models, so nothing is invented). This lets maxCostUsd bound worker spend too.
      cfg.store.finishAttempt(attemptId, { status: status === 'ok' ? 'passed' : 'failed', exitCode: supervised?.code ?? 0,
        durationMs: supervised?.durationMs, inputTokens, outputTokens,
        costUsd: usage.costUsd ?? (priceUsd(route.model, usage, cfg) || null) })
    }
    // Feed measured usage back into the per-(cli, model, effort) calibration so future runs'
    // `est.` column reflects this machine's real history rather than the static tier curve.
    if (reportedTokens > 0) cfg.store?.recordCalibration?.({ cli, model: route.model, effort: route.effort ?? 'low', tier: route.tier ?? 'simple', tokens: reportedTokens })
    ...
    return impl(status, wt, br, changed, gate_results, `attempt ${attempt} on ${cli}`, reportedTokens)
