...
export async function runImplementation(cfg, t, cli, attempt, feedback) {
  ...
  try {
    ...
    if (attemptId) {
      // Record both token directions. inputTokens was previously never persisted, and outputTokens
      // only read usage.totalTokens — missing the common {input_tokens,output_tokens} shape. Worker
      // USD cost is only known when the adapter reports it (external CLIs run on the user's own
      // model/subscription, whose price ultraswarm cannot infer), so costUsd is passed through as-is
      // and may be undefined for token-only workers (#ST5).
      const usage = supervised?.usage ?? {}
      const inputTokens = usage.input_tokens ?? usage.inputTokens ?? null
      const outputTokens = usage.output_tokens ?? usage.outputTokens ?? usage.totalTokens ?? parseTokens(out)
