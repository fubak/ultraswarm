      ...
      await gate(async () => {
        if (cfg.workerManager) {
          supervised = await cfg.workerManager.get(cli).execute({ task: t, cwd: wt, timeoutMs, env: allowedEnv(cfg), label: `${cfg.runId ?? 'run'}-${t.id}-${attempt}`,
            onStart: ({ pid, logPath }) => { if (cfg.store && cfg.runId) attemptId = cfg.store.startAttempt({ runId: cfg.runId, taskId: t.id, number: attempt, worker: cli, model: t.model_tier, pid, logPath }) } })
