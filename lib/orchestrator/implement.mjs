...
export async function runImplementation(cfg, t, cli, attempt, feedback) {
  ...
  try {
    ...
    if (attemptId) {
      // Worker USD cost is only known when the adapter reports it (external CLIs run on the user's own
      // model/subscription, whose price ultraswarm cannot infer), so costUsd is passed through as-is.
      cfg.store.finishAttempt(attemptId, { status: status === 'ok' ? 'passed' : 'failed', exitCode: supervised?.code ?? 0,
        durationMs: supervised?.durationMs, inputTokens, outputTokens, costUsd: usage.costUsd })
    }
