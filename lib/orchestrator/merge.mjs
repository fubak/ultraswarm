...
export async function mergeWave(cfg, agent, approved) {
  ...
  for (const r of approved) {
    try {
      // Capture stderr: `git merge --squash` prints "Automatic merge went well; stopped before
      // committing as requested" per task, which floods the live stream once a swarm has many tasks.
      execFileSync('git', ['merge', '--squash', '--', r.impl.branch], { cwd: target, stdio: ['ignore', 'pipe', 'pipe'] })
    } catch {
    ...
    //
    // If the squash staged nothing (the branch had no net change vs the target — e.g. a sibling task
    // already landed the identical diff), committing would fail. Record a clean no-op instead of
    // throwing, which would otherwise blow up the whole wave and every later wave (#O2).
    // `git diff --cached --quiet` exits 0 when nothing is staged, non-zero when there is a staged diff.
    const hasStaged = (() => { try { execFileSync('git', ['diff', '--cached', '--quiet'], { cwd: target }); return false } catch { return true } })()
    if (!hasStaged) {
      execFileSync('git', ['reset', '--hard', 'HEAD'], { cwd: target })
