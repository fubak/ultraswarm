...
test('buildReport summarizes merged, failed, the winning worker, and tokens saved', () => {
  ...
  assert.match(r, /t1\s+codex\s+integrated ✓/)
...
test('buildReport renders blocked tasks, attempts, per-worker contribution, and coverage (#10/#11)', () => {
  ...
  assert.match(r, /t3\s+—\s+blocked — dependency t2 did not merge/)
...
test('default report is plain terminal text; --markdown restores GitHub markdown; wall-clock shows when measured', () => {
  // WHY: the report is read raw in a terminal — plain UPPERCASE section headers, no #/##/**/_ that
  // would show as literal characters. markdown:true (CLI --markdown) restores markdown for PR pasting.
  const base = { merged: [{ task: 'a', cli: 'codex', merged: true }], taskCount: 1, runWallMs: 221000 }
  const plain = buildReport(base)
  assert.match(plain, /^ULTRASWARM RUN REPORT/)
  assert.match(plain, /\nSUMMARY\n/)
  assert.match(plain, /\nWORK OFFLOADED\n/)
  assert.doesNotMatch(plain, /^(#|\*\*|_)/m)   // no markdown header/bold/italic line-leads in plain mode
  // WHY: run wall-clock — the "how long did it take" number — appears in Summary when measured.
  assert.match(plain, /Wall-clock: 3m 41s\./)

  const md = buildReport({ ...base, markdown: true })
  assert.match(md, /^# ultraswarm run report/)
  assert.match(md, /## Summary/)
  assert.match(md, /## Work offloaded/)
  assert.match(md, /\*\*✓ All 1 task integrated/)
  assert.match(md, /_Staged on this run's integration branch/)
})
...
test('buildReport headline accounts for every task, including a "not integrated" post-merge regression', () => {
  ...
  assert.match(r, /c\s+codex\s+NOT integrated — post-merge gate regression/)
})
...
// v3.6: the report ENDS with an estimated-vs-used table by CLI × model × effort.
test('buildReport ends with the tokens-by-CLI/model/effort table: est vs used, honest "—" for unreported', () => {
  const r = buildReport({
    merged: [{ task: 't1', cli: 'codex', merged: true }], taskCount: 1,
    routeUsage: [
      { cli: 'codex', model: 'gpt-5.5', effort: 'high', estimated: 100000, used: 150000, attempts: 2 },
      { cli: 'gemini', model: 'gemini-2.5-pro', effort: 'low', estimated: 30000, used: null, attempts: 1 },
    ],
  })
  // WHY: this table is the user-facing deliverable — estimated AND used per (CLI, model, effort).
  assert.match(r, /TOKENS BY CLI \/ MODEL \/ EFFORT/)
  assert.match(r, /codex\s+gpt-5\.5\s+high\s+100,000\s+150,000\s+\+50%\s+2/)
  // WHY: a CLI that reported no structured usage shows "—", never a number backfilled from the
  // estimate (Rule 12 / v3.5.13 honesty invariant).
  assert.match(r, /gemini\s+gemini-2\.5-pro\s+low\s+30,000\s+—\s+—\s+1/)
  // Total row sums estimates; its Δ compares only buckets that actually reported usage.
  assert.match(r, /Total\s+130,000\s+150,000\s+\+50%\s+3/)
  assert.match(r, /1 route\(s\) reported no usage/)
  // The table is the LAST section of the report.
  const tail = r.split('TOKENS BY CLI / MODEL / EFFORT')[1]
  assert.ok(tail && !/WORK OFFLOADED|SUMMARY/.test(tail), 'route table must end the report')
})

// v3.6: ledger invariant — task rows must reconcile with the plan's task count.
test('buildReport prints a loud LEDGER MISMATCH line when task rows do not add up to taskCount', () => {
  const r = buildReport({ merged: [{ task: 't1', cli: 'codex', merged: true }], taskCount: 3 })
  assert.match(r, /LEDGER MISMATCH: 1 task rows but the plan had 3/)
  const ok = buildReport({ merged: [{ task: 't1', cli: 'codex', merged: true }], taskCount: 1 })
  assert.doesNotMatch(ok, /LEDGER MISMATCH/)
})
