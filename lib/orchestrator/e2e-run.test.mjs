...
async function drive({ repo, home }, argv) {
  ...
  try {
    const code = await commandMain(argv, repo)
...
describe('e2e orchestrator: run → status → logs → export → merge (in-process, mock brain)', () => {
  it('drives a complex multi-wave plan to awaiting_merge, then merges it into the target repo', async () => {
    // enabled: two built-ins (codex, droid) stubbed to the fake worker, plus an ALIAS (local-x)
    // extending `pi`. The alias inherits pi's "general/full-stack/refactors" strengths, so a task
    // described as plain "general" work routes to the alias (codex/droid lack the 'general'
    // strength). NOTE: a plan task may NOT set `cli` to an alias — plan-schema validates task.cli
    // against the built-in DEFAULT_REGISTRY only — so the alias is reached via AUTO-ROUTING, not an
    // explicit task.cli (see report for this intentional deviation from the literal brief).
    const config = {
      enabled: ['codex', 'droid', 'local-x'],
      overrides: {
        codex: { invocation: FAKE_WORKER },
        droid: { invocation: FAKE_WORKER },
      },
      aliases: {
        'local-x': {
          extends: 'pi',
          specialty: 'general local generalist',
          models: { simple: { model: 'mock-local', invocation: FAKE_WORKER } },
        },
      },
    }
    ...
    // Descriptions steer routing: "refactor/architecture" → codex/droid; bare "general" → local-x.
    const plan = {
      tasks: [
        { id: 'core-refactor', description: 'refactor the core module architecture', files: ['core.js'], complexity_score: 15, risk: 'routine', dependencies: [], prompt: 'refactor core' },
        { id: 'general-setup', description: 'general project setup work', files: ['setup.js'], complexity_score: 10, risk: 'routine', dependencies: [], prompt: 'general setup' },
        { id: 'api-layer', description: 'full-stack refactor of the api layer', files: ['api.js'], complexity_score: 20, risk: 'routine', dependencies: ['core-refactor'], prompt: 'build api on core' },
    ...
    try {
      ...
      assert.match(run.stdout, /4\/4 merged/, 'all four tasks integrated into the integration branch')
      for (const id of ['core-refactor', 'general-setup', 'api-layer', 'general-polish']) {
        assert.match(run.stdout, new RegExp(`\\| ${id} \\| merged`), `report marks ${id} merged`)
      }
      ...
      // The printed plan JSON carries each task's resolved worker.
      assert.match(run.stdout, /"worker": "local-x"/, 'a task auto-routed to the local-x alias')
      ...
      // ── merge ───────────────────────────────────────────────────────────────────
      const merged = await drive(env, ['merge', RID, '--approve'])
      assert.equal(merged.code, EXIT.OK, `merge should succeed; output:\n${merged.stdout}`)
      const mergeResult = JSON.parse(merged.stdout)
      assert.equal(mergeResult.runId, RID, 'merge result carries the run id')
  ...
  it('holds the approval gate: run WITHOUT --approve-plan returns EXIT.APPROVAL and executes nothing', async () => {
    const config = {
      enabled: ['codex', 'droid'],
      overrides: { codex: { invocation: FAKE_WORKER }, droid: { invocation: FAKE_WORKER } },
    }
    const env = makeEnv(config)
    const RID = 'e2e-approval-gate'
    const plan = {
      tasks: [
        { id: 't1', description: 'refactor a thing', files: ['a.js'], complexity_score: 10, risk: 'routine', dependencies: [], prompt: 'do it' },
      ],
    }
    const planFile = writePlan(env.home, plan)
    try {
  ...
  it('enforces forbiddenPaths against a file the worker writes into a brand-new subdirectory', async () => {
    ...
    const config = {
      enabled: ['codex'],
      overrides: { codex: { invocation: ROGUE_WORKER } },
      policy: { forbiddenPaths: ['**/*.secret'], minimumHealthyWorkers: 1 },
    }
    const env = makeEnv(config)
    const RID = 'e2e-forbidden-newdir'
    const plan = { tasks: [
      { id: 'rogue-task', description: 'write app.js', files: ['app.js'], cli: 'codex', complexity_score: 10, risk: 'routine', dependencies: [], prompt: 'write app.js' },
    ] }
    const planFile = writePlan(env.home, plan)
    try {
      const run = await drive(env, ['run', '--plan-file', planFile, '--approve-plan', '--run-id', RID, '--worktree-root', env.worktreeRoot])
  ...
  it('surfaces a failure: a task whose worker produces no change fails after retries/escalation', async () => {
    // codex => the working fake worker; droid => the no-op worker (prints a token line, writes
    // nothing). The plan routes the bad task explicitly to droid (a built-in, so plan-schema
    // permits the explicit cli). The runner's mechanical "diff must touch the declared files" gate
    // (and the implement no_changes gate) reject every attempt; after 3 escalating attempts the
    // task tombstones. This exercises the implement retry/escalation loop and report's failed path.
    const config = {
      enabled: ['codex', 'droid'],
      overrides: { codex: { invocation: FAKE_WORKER }, droid: { invocation: NOOP_WORKER } },
    }
    const env = makeEnv(config)
    const RID = 'e2e-failure-path'
    const plan = {
      tasks: [
        { id: 'noop-task', description: 'full-stack change that the worker silently skips', files: ['ghost.js'], cli: 'droid', complexity_score: 10, risk: 'routine', dependencies: [], prompt: 'write ghost.js' },
      ],
    }
    const planFile = writePlan(env.home, plan)
    try {
      ...
      assert.match(run.stdout, /noop-task \| FAILED/, 'report marks the task FAILED (exhausted)')
      assert.match(run.stdout, /0\/1 merged/, 'nothing merged')
