...
// ── alias workers must be eligible for high-risk competition ─────────────────────────────────────
// Bug: cliUsable accepted only built-in ALLOWED_CLIS or legacy `registry` entries, not cfg.aliases —
// so a high-risk task pinned to an alias worker tombstoned as "only N usable worker(s)" even though
// the aliases route fine for routine tasks.
describe('alias workers in high-risk competition', () => {
  test('a high-risk task competes when its competitors are alias workers (not built-ins)', async () => {
    const task = makeTask({ id: 'h', model_tier: 'complex', complexity_score: 60, risk: 'high', cli: 'al-1' })
    const agent = (l) => {
      if (l.startsWith('judge:'))  return judgeScore()
      if (l.startsWith('verify:')) return passVerdict()
      return approveReview
    }
    const { result, calls } = await runTask(task, {
      registry: {},   // no legacy registry → aliases are the ONLY way these names resolve
      aliases: { 'al-1': { extends: 'codex', binary: 'node' }, 'al-2': { extends: 'codex', binary: 'node' } },
      alternates: { 'al-1': 'al-2', 'al-2': 'al-1' },
    }, agent, () => okImpl())
    assert.ok(!result.failed, `alias competitors must not tombstone; got: ${JSON.stringify(result)}`)
    const impls = calls.filter(c => c.isImpl)
    assert.equal(impls.length, 2, 'both alias competitors run an implementation')
    assert.equal(new Set(impls.map(c => c.label.split(':')[2])).size, 2, 'two distinct alias CLIs competed')
  })
})
...
describe('competition result reporting and retry feedback', () => {
  ...
  test('a QA-rejected competition winner retries WITH the QA rejection feedback (#O3)', async () => {
    ...
    const { logs } = await runTask(highTask, {}, rejectingAgent, recordingImpl)
    ...
    // WHY: the rejection must also be VISIBLE in the live stream. A gate-passing winner that QA
    // rejects previously jumped silently to "attempt 2" (the operator saw "✓ gates" then a fresh
    // attempt with no reason). The stream must name the rejection so the retry is explicable.
    assert.ok(logs.some((l) => /✗ h: competition winner \(.+\) rejected by QA \(\d+ issue/.test(l)),
      `the QA rejection of the competition winner must be logged; got logs:\n${logs.join('\n')}`)
  })
})
