...
export function createIntegrationWorktree({ repo, runId, baseSha, worktreeRoot }) {
  ...
  // Capture git's stderr ("Preparing worktree …") instead of inheriting it, so the integration
  // worktree setup stays out of the live progress stream (see implement.mjs for the per-task case).
  git(repo, ['worktree', 'add', worktree, '-b', branch, baseSha], { stdio: ['ignore', 'pipe', 'pipe'] })
