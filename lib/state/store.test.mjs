...
// v3: fresh dbs get tier/effort columns on attempts + the route_calibration table via BASE_TABLES.
test('fresh store has v3 attempts columns and route_calibration table', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'us-state-'))
  const store = new StateStore(path.join(dir, 'state.sqlite'))
  const cols = store.db.prepare('PRAGMA table_info(attempts)').all().map((c) => c.name)
  assert.ok(cols.includes('tier') && cols.includes('effort'))
  assert.ok(store.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='route_calibration'").get())
  assert.equal(store.db.prepare('SELECT version FROM schema_meta WHERE id=1').get().version, 3)
  store.close()
})

// v3 upgrade: a pre-v3 db (v2-shaped attempts, no tier/effort, no route_calibration) must gain
// both on open, without losing existing data.
test('v2 db upgrades to v3: gains tier/effort columns and route_calibration table', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'us-state-'))
  const file = path.join(dir, 'state.sqlite')
  ...
  bootstrap.exec(`
    ...
    INSERT INTO runs(id,repo,base_sha,status,plan_json,policy_json,created_at,updated_at)
      VALUES('r-old','/tmp/repo','abc','merged','{}','{}','now','now');
    INSERT INTO attempts(run_id,task_id,number,worker,model,status,started_at)
      VALUES('r-old','a',1,'codex','m','passed','now');
  `)
  bootstrap.prepare('INSERT INTO schema_meta(id,version) VALUES(1,2)').run()
  bootstrap.close()
  ...
  assert.equal(store.getRun('r-old').id, 'r-old')
  assert.equal(store.getAttempts('r-old')[0].model, 'm')
  store.close()
})

// startAttempt with tier/effort: persisted and retrievable via getAttempts.
test('startAttempt persists tier and effort when provided', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'us-state-'))
  const store = new StateStore(path.join(dir, 'state.sqlite'))
  store.createRun({ id: 'r1', repo: dir, baseSha: 'abc', plan: { tasks: [{ id: 'a' }] }, policy: {}, waves: [[{ id: 'a' }]] })
  store.startAttempt({ runId: 'r1', taskId: 'a', number: 1, worker: 'codex', model: 'm', tier: 'complex', effort: 'high' })
  const row = store.getAttempts('r1')[0]
  assert.equal(row.tier, 'complex')
  assert.equal(row.effort, 'high')
  store.close()
})

// startAttempt without tier/effort: stays backward compatible, stores nulls.
test('startAttempt without tier/effort stores nulls (backward compat)', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'us-state-'))
  const store = new StateStore(path.join(dir, 'state.sqlite'))
  store.createRun({ id: 'r1', repo: dir, baseSha: 'abc', plan: { tasks: [{ id: 'a' }] }, policy: {}, waves: [[{ id: 'a' }]] })
  store.startAttempt({ runId: 'r1', taskId: 'a', number: 1, worker: 'codex', model: 'm' })
  const row = store.getAttempts('r1')[0]
...
// recordCalibration: two calls for the same (cli,model,effort,tier) upsert-accumulate.
test('recordCalibration upserts attempts and total_tokens across calls', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'us-state-'))
  const store = new StateStore(path.join(dir, 'state.sqlite'))
  store.recordCalibration({ cli: 'codex', model: 'gpt-5', effort: 'medium', tier: 'moderate', tokens: 1000 })
  store.recordCalibration({ cli: 'codex', model: 'gpt-5', effort: 'medium', tier: 'moderate', tokens: 500 })
  const rows = store.getCalibration()
  assert.equal(rows.length, 1)
  assert.equal(rows[0].attempts, 2)
  assert.equal(rows[0].total_tokens, 1500)
  store.close()
})

test('getCalibration returns rows ordered by cli,model,effort', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'us-state-'))
  const store = new StateStore(path.join(dir, 'state.sqlite'))
  store.recordCalibration({ cli: 'opencode', model: 'gpt-5', effort: 'low', tier: 'simple', tokens: 100 })
  store.recordCalibration({ cli: 'codex', model: 'gpt-5', effort: 'medium', tier: 'moderate', tokens: 200 })
  const rows = store.getCalibration()
  assert.equal(rows.length, 2)
  assert.equal(rows[0].cli, 'codex')
  assert.equal(rows[1].cli, 'opencode')
  store.close()
})
...
test('migrate() loop body runs when stored version is behind SCHEMA_VERSION', () => {
  ...
  bootstrap.exec(`
    ...
    CREATE TABLE IF NOT EXISTS brain_costs (id INTEGER PRIMARY KEY AUTOINCREMENT, run_id TEXT NOT NULL,
      cost_usd REAL NOT NULL, created_at TEXT NOT NULL);
  `)
  // Set stored version to 0 — below SCHEMA_VERSION — so migrate() loop runs.
  bootstrap.prepare('INSERT INTO schema_meta(id,version) VALUES(1,0)').run()
  bootstrap.close()
  // Opening StateStore must succeed and bump version from 0 to SCHEMA_VERSION via the migration
  // loop, adding the orchestrator columns (v2) and tier/effort + route_calibration (v3).
  const store = new StateStore(file)
  assert.equal(store.db.prepare('SELECT version FROM schema_meta WHERE id=1').get().version, 3)
  ...
  const attemptCols = store.db.prepare('PRAGMA table_info(attempts)').all().map((c) => c.name)
  assert.ok(attemptCols.includes('tier') && attemptCols.includes('effort'), 'v3 migration added tier/effort columns')
  store.close()
})
