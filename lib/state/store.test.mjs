...
test('migrate() loop body runs when stored version is behind SCHEMA_VERSION', () => {
  // WHY: with stored=0 and SCHEMA_VERSION=1, the loop executes for v=1; step=MIGRATIONS[1]=undefined,
  // so the `if (step)` branch is false and only the UPDATE runs. Proves lines 99-104 are reachable.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'us-state-'))
  const file = path.join(dir, 'state.sqlite')
  ...
  bootstrap.close()
  // Opening StateStore must succeed and bump version from 0 to 1 via the migration loop.
  const store = new StateStore(file)
  assert.equal(store.db.prepare('SELECT version FROM schema_meta WHERE id=1').get().version, 1)
  store.close()
})
