...
// Minimal fixed-width table. Columns auto-size to the widest cell. `align` is an optional per-column
// array of 'left' | 'right' (default 'left') — right-align numeric columns so digits line up. The ─
// separator row always fills the full column width regardless of alignment.
export function table(headers, rows, align = []) {
  ...
  const cell = (c, i) => (align[i] === 'right' ? String(c ?? '').padStart(widths[i]) : String(c ?? '').padEnd(widths[i]))
  const fmt = (cells) => cells.map(cell).join('  ').trimEnd()
...
// worker_metrics rows → a WORKER TRACK RECORD table for doctor. '' when there are no rows yet
// (a fresh repo has recorded no outcomes), so callers can skip the section entirely.
export function renderMetrics(metrics = []) {
  if (!metrics.length) return ''
  const rows = metrics.map((m) => [m.worker, m.task_class, m.runs, `${Math.round((m.passes / m.runs) * 100)}%`])
  return ['WORKER TRACK RECORD', table(['worker', 'class', 'runs', 'pass %'], rows, [undefined, undefined, 'right', 'right'])].join('\n')
}

// `doctor --models`: resolved model per CLI per tier, so a stale pin is visible at a glance.
export function renderModels(rows = []) {
  return table(['CLI', 'simple', 'moderate', 'complex', 'expert'], rows.map((r) => [r.cli, r.simple, r.moderate, r.complex, r.expert]))
}
