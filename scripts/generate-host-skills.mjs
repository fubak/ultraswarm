...
1. Functionally verify the worker pool: \`node <root>/${contract.runner} preflight\`. This runs a cached smoke test (not just \`--version\`) and prints a roster marking each CLI INSTALLED and FUNCTIONAL. Drop any worker shown UNUSABLE — do not pin a task to it — and confirm at least the policy minimum of functional workers remain.
...
3. Preview with \`node <root>/${contract.runner} run --plan-file .ultraswarm-plan.json\`. Show tasks, waves, routing explanations, policy findings, and gates. Obtain explicit plan approval.
4. Execute with \`node <root>/${contract.runner} run --plan-file .ultraswarm-plan.json --approve-plan\`, keeping the runner's stderr visible and relaying its live progress to the user as it streams: wave headers, per-agent dispatch lines, gate results, and the periodic active/idle heartbeat that shows every worker's state.
...
6. Commit any working-tree changes first, then obtain separate merge approval and run \`node <root>/${contract.runner} merge <run-id> --approve\`.
